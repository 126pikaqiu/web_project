let itemNum = 0;
let pageNum = 1;
let layLocation = [0,3,6,1,4,7,2,5,8];
    ...
    $.when(searchByOrder(data))
        .done(function (msg) {
            ...
            for (let i in msg) {
                ...
                $($("button.lay-bt")[i]).attr("onclick","opsn(" + msg[i]["id"] + ")");
                $($("button.lay-bt2")[i]).attr("onclick","opsn(" + msg[i]["id"] + ")");
            ...
            let itemsNumber = itemNum;
            let allPageNumber = itemsNumber % 9 === 0 ? itemsNumber / 9 : parseInt(itemsNumber / 9) + 1;
            $(".input-txt").attr("max",allPageNumber);
            $(".lay-footer .p-skip em b").html(allPageNumber);
            if (allPageNumber < 12 || pageNum > 12) {
                for (let i = 0; i < allPageNumber % 12; i++) {
                    $($(".paN")[i]).removeClass("dis-none");
                }
            }else {
                $(".paN").removeClass("dis-none");
            }
